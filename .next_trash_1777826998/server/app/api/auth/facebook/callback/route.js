(() => {
    ...
    (a.modules = {
      ...
      29159: (a, b, c) => {
        ...
        async function y(a) {
          let b =
            ...
            c =
              ...
              `${b.replace(/\/$/, "")}/api/auth/facebook/callback`,
            d = process.env.FACEBOOK_APP_ID,
            e = process.env.FACEBOOK_APP_SECRET,
            f = a.nextUrl,
            g = f.searchParams.get("code"),
            h = f.searchParams.get("state"),
            i = a.cookies.get("fb_oauth_state")?.value;
          if (!d || !e || !g || !h || h !== i)
            return u.NextResponse.redirect(new URL("/dashboard/meta-connect?error=oauth", b));
          if (!process.env.JWT_SECRET || !process.env.ENCRYPTION_KEY)
            return u.NextResponse.redirect(new URL("/dashboard/meta-connect?error=secrets", b));
          try {
            let f = await (0, v.J6)({ clientId: d, clientSecret: e, redirectUri: c, code: g }),
              h = new URL("https://graph.facebook.com/v18.0/me");
            (h.searchParams.set("fields", "id,name,email"),
              h.searchParams.set("access_token", f.access_token));
            let i = await fetch(h.toString(), { cache: "no-store" }),
              j = await i.json();
            if (!j?.id)
              return u.NextResponse.redirect(new URL("/dashboard/meta-connect?error=profile", b));
            let k = a.cookies.get("fb_oauth_app_user")?.value,
              l = await (0, w.TP)({
                accessToken: f.access_token,
                fbUserId: j.id,
                name: j.name,
                email: j.email,
                appUserId: k && k.length > 0 ? k : void 0,
              }),
              m = await (0, x.Mp)({ agencyId: l }),
              n = u.NextResponse.redirect(new URL("/dashboard/meta-connect?connected=1", b));
            return (
              n.cookies.set(x.z4, m, {
                httpOnly: !0,
                secure: !0,
                sameSite: "lax",
                path: "/",
              ...
              n.cookies.delete("fb_oauth_state"),
              n.cookies.set("fb_oauth_app_user", "", {
                httpOnly: !0,
                secure: !0,
                path: "/",
                maxAge: 0,
            ...
            return u.NextResponse.redirect(new URL("/dashboard/meta-connect?error=token", b));
          }
        }
        let z = new e.AppRouteRouteModule({
            definition: {
              kind: f.RouteKind.APP_ROUTE,
              page: "/api/auth/facebook/callback/route",
              pathname: "/api/auth/facebook/callback",
              filename: "route",
              bundlePath: "app/api/auth/facebook/callback/route",
            },
            distDir: ".next",
            relativeProjectDir: "",
            resolvedPagePath:
              "/media/iqbalxrr/Project Disk/Agency Work/ad_reportly/src/app/api/auth/facebook/callback/route.ts",
            nextConfigOutput: "",
            userland: d,
          }),
        ...
        async function E(a, b, c) {
          var d;
          let e = "/api/auth/facebook/callback/route";
          "/index" === e && (e = "/");
  ...
  b.C(a);
  var c = b.X(0, [331, 692, 840], () => b((b.s = 29159)));
  module.exports = c;
})();
