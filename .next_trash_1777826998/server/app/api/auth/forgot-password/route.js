(() => {
    ...
    (a.modules = {
      ...
      4797: (a, b, c) => {
        ...
        var e = c(95736),
          ...
          x = c(72977),
          y = c(77492);
        let z = w.Ik({ email: w.Yj().trim().email() });
        async function A(a) {
          let b;
          try {
            b = await a.json();
          } catch {
            return v.NextResponse.json({ error: "Invalid JSON" }, { status: 400 });
          }
          let c = z.safeParse(b);
          if (!c.success) return v.NextResponse.json({ error: "Invalid email" }, { status: 400 });
          try {
            await (0, x.I)();
          } catch {
            return v.NextResponse.json({ ok: !0 });
          }
          let d = c.data.email.toLowerCase(),
            e = (0, u.randomBytes)(32).toString("hex"),
            f = new Date(Date.now() + 36e5);
          return (
            await y.F.updateOne(
              { email: d },
              { $set: { resetPasswordToken: e, resetPasswordExpires: f } },
            ),
            v.NextResponse.json({ ok: !0 })
          );
        }
        let B = new e.AppRouteRouteModule({
            ...
            relativeProjectDir: "",
            resolvedPagePath:
              "/media/iqbalxrr/Project Disk/Agency Work/ad_reportly/src/app/api/auth/forgot-password/route.ts",
            nextConfigOutput: "",
            userland: d,
          }),
        ...
        async function G(a, b, c) {
          var d;
          let e = "/api/auth/forgot-password/route";
          "/index" === e && (e = "/");
  ...
  b.C(a);
  var c = b.X(0, [331, 692, 711], () => b((b.s = 4797)));
  module.exports = c;
})();
