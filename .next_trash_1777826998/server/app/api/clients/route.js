(() => {
    ...
    (a.modules = {
      ...
      1949: (a, b, c) => {
        ...
        async function x(a) {
          ...
          try {
            let a = (await (0, w.tg)(b)).map((a) => {
              let b = a.email.split("@")[0] ?? a.email,
                c = b.replace(/[._]/g, " ").split(" ").filter(Boolean),
                d = c.length >= 2 ? (c[0][0] + c[1][0]).toUpperCase() : b.slice(0, 2).toUpperCase();
              return {
                id: a.email,
                initials: d,
                name: b,
                ...
                email: a.email,
                accounts: a.shareCount,
                status: "active",
                lastShared: a.lastShared.toISOString(),
              };
            });
            return u.NextResponse.json({ success: !0, clients: a });
          } catch (b) {
            let a = b instanceof Error ? b.message : "Server error";
            return u.NextResponse.json({ success: !1, error: a }, { status: 500 });
          }
        }
        let y = new e.AppRouteRouteModule({
            ...
            relativeProjectDir: "",
            resolvedPagePath:
              "/media/iqbalxrr/Project Disk/Agency Work/ad_reportly/src/app/api/clients/route.ts",
            nextConfigOutput: "",
            userland: d,
          }),
  ...
  b.C(a);
  var c = b.X(0, [331, 692, 28, 271, 840], () => b((b.s = 1949)));
  module.exports = c;
})();
