(() => {
    ...
    (a.modules = {
      ...
      9729: (a, b, c) => {
        ...
        function z(a) {
          ...
          let b = 0;
          for (let c of a) y.has(c.action_type) && (b += parseInt(c.value, 10) || 0);
          return b;
        }
        function A(a) {
          if (!a?.length) return 0;
          let b = 0;
          for (let c of a) y.has(c.action_type) && (b += parseFloat(c.value) || 0);
          return b;
        }
        function B(a) {
          let b = a.trim().split(/\s+/).filter(Boolean);
          return b.length >= 2
            ? (b[0][0] + b[1][0]).toUpperCase().slice(0, 2)
            : (b[0] ?? a)
                .replace(/[^a-zA-Z0-9]/g, "")
                .slice(0, 2)
                .toUpperCase() || "?";
        }
        let C = ["primary", "dark", "muted"];
        async function D(a) {
          let b = await (0, v.E)(a);
          if (!b)
          ...
          let c = await (0, w.yD)(b),
            d = (a, b = "BDT", c = "৳") =>
              u.NextResponse.json({
                ...
                recentCampaigns: [],
                campaigns: [],
                kpis: { totalSpend: 0, conversions: 0, avgRoas: "—", avgCpc: "—" },
              });
          if (!c) return d(!1);
          let e = [];
          try {
            e = (await (0, x.eP)(c)).data ?? [];
          } catch (b) {
            let a = b instanceof Error ? b.message : "Facebook API error";
            return u.NextResponse.json({ success: !1, error: a }, { status: 502 });
          }
          let f = e[0]?.currency ?? "BDT",
            g = (function (a) {
              if (!a) return "৳";
              let b = a.toUpperCase();
              return "USD" === b ? "$" : "EUR" === b ? "€" : "BDT" === b ? "৳" : `${b} `;
            })(f);
          if (0 === e.length) return d(!0, f, g);
          let h = [],
            i = 0,
            j = 0,
            k = 0,
          ...
          for (let a of e)
            try {
              let [b, d, e, f] = await Promise.all([
                (0, x.jc)(c, a.id),
                (0, x.rd)(c, a.id),
                (0, x._w)(c, a.id),
                (0, x.ZF)(c, a.id),
              ]);
              for (let a of (b.length && h.push(b),
              d &&
                ((i += parseFloat(d.spend ?? "0") || 0),
                (j += z(d.actions)),
                (k += A(d.action_values)),
                (l += parseInt(d.clicks ?? "0", 10) || 0)),
              e)) {
                let b = a.campaign_id ?? "";
                if (!b) continue;
                let c = parseFloat(a.spend ?? "0") || 0,
                  d = parseInt(a.clicks ?? "0", 10) || 0,
                  e = parseInt(a.impressions ?? "0", 10) || 0,
                  g = z(a.actions),
                  h = A(a.action_values),
                  i = f.get(b) ?? "",
                  j = m.get(b);
                j
                  ? ((j.spend += c),
                    ...
                    (j.conv += g),
                    (j.purchaseValue += h),
                    !j.effectiveStatus && i && (j.effectiveStatus = i))
                  : m.set(b, {
                      id: b,
                      name: a.campaign_name ?? "Campaign",
                      spend: c,
                      clicks: d,
                      impressions: e,
                      conv: g,
          ...
          let n = (function (a) {
              ...
              for (let c of a)
                for (let a of c) {
                  let c = a.date_start,
                    d = b.get(c) ?? { spend: 0, clicks: 0 };
                  ((d.spend += parseFloat(a.spend ?? "0") || 0),
                    (d.clicks += parseInt(a.clicks ?? "0", 10) || 0),
                    b.set(c, d));
                }
              return [...b.entries()]
                ...
                .map(([a, b]) => ({
                  date: a,
                  label: new Date(a + "T12:00:00Z").toLocaleDateString("en-US", {
                    month: "short",
                    day: "numeric",
                  }),
                  spend: Math.round(b.spend),
                  results: Math.round(b.clicks),
                }));
            })(h),
            o = [...m.values()].sort((a, b) => b.spend - a.spend),
            p = e.length,
            q = o
              .slice(0, 5)
              .map((a, b) => ({
            ...
            r = (a) => {
              let b = a.spend > 0 && a.purchaseValue > 0 ? a.purchaseValue / a.spend : 0,
                c = a.effectiveStatus
                  ? (function (a) {
                      let b = a.toUpperCase();
                      return "ACTIVE" === b
                        ? "active"
                        : "PAUSED" === b || "ADSET_PAUSED" === b || "CAMPAIGN_PAUSED" === b
                          ? "paused"
                          : "ARCHIVED" === b || "DELETED" === b
                            ? "completed"
                            : "other";
                    })(a.effectiveStatus)
                  : a.spend > 0
                    ? "active"
                    : "paused",
                d = a.impressions > 0 ? (a.clicks / a.impressions) * 100 : 0,
                e = a.clicks > 0 ? a.spend / a.clicks : 0;
              return {
                id: a.id,
                code: B(a.name),
                name: a.name,
                accounts: p,
                spend: a.spend,
                results: a.conv > 0 ? a.conv : a.clicks,
                roas: b,
                status: c,
                ctr: d,
                cpc: e,
            ...
            s = o.slice(0, 8).map(r),
            t = o.map(r),
            y = i > 0 && k > 0 ? `${(k / i).toFixed(2)}\xd7` : "—",
            D = l > 0 && i > 0 ? `${g}${(i / l).toFixed(2)}` : "—";
          return u.NextResponse.json({
            ...
            recentCampaigns: s,
            campaigns: t,
            kpis: { totalSpend: i, conversions: j, avgRoas: y, avgCpc: D },
          });
        }
        let E = new e.AppRouteRouteModule({
            definition: {
              kind: f.RouteKind.APP_ROUTE,
              page: "/api/dashboard/overview/route",
              pathname: "/api/dashboard/overview",
              filename: "route",
              bundlePath: "app/api/dashboard/overview/route",
            },
            distDir: ".next",
            relativeProjectDir: "",
            resolvedPagePath:
              "/media/iqbalxrr/Project Disk/Agency Work/ad_reportly/src/app/api/dashboard/overview/route.ts",
            nextConfigOutput: "",
            userland: d,
          }),
          { workAsyncStorage: F, workUnitAsyncStorage: G, serverHooks: H } = E;
        function I() {
          return (0, g.patchFetch)({ workAsyncStorage: F, workUnitAsyncStorage: G });
        }
        async function J(a, b, c) {
          var d;
          let e = "/api/dashboard/overview/route";
          "/index" === e && (e = "/");
          let g = await E.prepare(a, b, { srcPage: e, multiZoneDraftMode: !1 });
          if (!g)
            return (
              (b.statusCode = 400),
              b.end("Bad Request"),
          ...
          let {
            ...
            F = !!(y.dynamicRoutes[D] || y.routes[C]);
          if (F && !x) {
            let a = !!y.routes[C],
              b = y.dynamicRoutes[D];
          ...
          let H = !0 === E.isDev || !F,
            ...
            M = {
              ...
              renderOpts: {
                ...
                onInstrumentationRequestError: (b, c, d) => E.onRequestError(a, b, d, z),
              },
  ...
  b.C(a);
  var c = b.X(0, [331, 692, 28, 271, 840], () => b((b.s = 9729)));
  module.exports = c;
})();
