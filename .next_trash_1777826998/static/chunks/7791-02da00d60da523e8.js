"use strict";
(self.webpackChunk_N_E = self.webpackChunk_N_E || []).push([
  [7791],
  {
    6191: (e, r, n) => {
      n.d(r, { A: () => t });
      let t = (0, n(5121).A)("plus", [
        ["path", { d: "M5 12h14", key: "1ays0h" }],
        ["path", { d: "M12 5v14", key: "s699le" }],
      ]);
    },
    15870: (e, r, n) => {
      n.d(r, { A: () => t });
      let t = (0, n(5121).A)("settings", [
        [
          "path",
          {
            d: "M9.671 4.136a2.34 2.34 0 0 1 4.659 0 2.34 2.34 0 0 0 3.319 1.915 2.34 2.34 0 0 1 2.33 4.033 2.34 2.34 0 0 0 0 3.831 2.34 2.34 0 0 1-2.33 4.033 2.34 2.34 0 0 0-3.319 1.915 2.34 2.34 0 0 1-4.659 0 2.34 2.34 0 0 0-3.32-1.915 2.34 2.34 0 0 1-2.33-4.033 2.34 2.34 0 0 0 0-3.831A2.34 2.34 0 0 1 6.35 6.051a2.34 2.34 0 0 0 3.319-1.915",
            key: "1i5ecw",
          },
        ],
        ["circle", { cx: "12", cy: "12", r: "3", key: "1v7zrd" }],
      ]);
    },
    59384: (e, r, n) => {
      n.d(r, { A: () => t });
      let t = (0, n(5121).A)("layout-dashboard", [
        ["rect", { width: "7", height: "9", x: "3", y: "3", rx: "1", key: "10lvy0" }],
        ["rect", { width: "7", height: "5", x: "14", y: "3", rx: "1", key: "16une8" }],
        ["rect", { width: "7", height: "9", x: "14", y: "12", rx: "1", key: "1hutg5" }],
        ["rect", { width: "7", height: "5", x: "3", y: "16", rx: "1", key: "ldoo1y" }],
      ]);
    },
    60186: (e, r, n) => {
      n.d(r, {
        H_: () => rf,
        UC: () => rd,
        YJ: () => rs,
        q7: () => rp,
        VF: () => rm,
        JU: () => rc,
        ZL: () => ri,
        z6: () => rv,
        hN: () => rh,
        bL: () => rl,
        wv: () => rg,
        Pb: () => rw,
        G5: () => rx,
        ZP: () => ry,
        l9: () => ru,
      });
      var t = n(12115),
        o = n(92556),
        a = n(94446),
        l = n(3468),
        u = n(23558),
        i = n(88142),
        d = n(79118),
        s = n(66218),
        c = n(44831),
        p = n(19526),
        f = n(69666),
        v = n(68946),
        h = n(66093),
        m = n(75433),
        g = n(76842),
        w = n(70222),
        y = n(95155),
        x = "rovingFocusGroup.onEntryFocus",
        b = { bubbles: !1, cancelable: !0 },
        C = "RovingFocusGroup",
        [M, j, R] = (0, d.N)(C),
        [k, D] = (0, l.A)(C, [R]),
        [_, A] = k(C),
        K = t.forwardRef((e, r) =>
          (0, y.jsx)(M.Provider, {
            scope: e.__scopeRovingFocusGroup,
            children: (0, y.jsx)(M.Slot, {
              scope: e.__scopeRovingFocusGroup,
              children: (0, y.jsx)(I, { ...e, ref: r }),
            }),
          }),
        );
      K.displayName = C;
      var I = t.forwardRef((e, r) => {
          let {
              __scopeRovingFocusGroup: n,
              orientation: l,
              loop: d = !1,
              dir: c,
              currentTabStopId: p,
              defaultCurrentTabStopId: f,
              onCurrentTabStopIdChange: v,
              onEntryFocus: h,
              preventScrollOnEntryFocus: m = !1,
              ...g
            } = e,
            M = t.useRef(null),
            R = (0, a.s)(r, M),
            k = (0, s.jH)(c),
            [D, A] = (0, u.i)({
              prop: p,
              defaultProp: null != f ? f : null,
              onChange: v,
              caller: C,
            }),
            [K, I] = t.useState(!1),
            E = (0, w.c)(h),
            P = j(n),
            S = t.useRef(!1),
            [N, F] = t.useState(0);
          return (
            t.useEffect(() => {
              let e = M.current;
              if (e) return (e.addEventListener(x, E), () => e.removeEventListener(x, E));
            }, [E]),
            (0, y.jsx)(_, {
              scope: n,
              orientation: l,
              dir: k,
              loop: d,
              currentTabStopId: D,
              onItemFocus: t.useCallback((e) => A(e), [A]),
              onItemShiftTab: t.useCallback(() => I(!0), []),
              onFocusableItemAdd: t.useCallback(() => F((e) => e + 1), []),
              onFocusableItemRemove: t.useCallback(() => F((e) => e - 1), []),
              children: (0, y.jsx)(i.sG.div, {
                tabIndex: K || 0 === N ? -1 : 0,
                "data-orientation": l,
                ...g,
                ref: R,
                style: { outline: "none", ...e.style },
                onMouseDown: (0, o.mK)(e.onMouseDown, () => {
                  S.current = !0;
                }),
                onFocus: (0, o.mK)(e.onFocus, (e) => {
                  let r = !S.current;
                  if (e.target === e.currentTarget && r && !K) {
                    let r = new CustomEvent(x, b);
                    if ((e.currentTarget.dispatchEvent(r), !r.defaultPrevented)) {
                      let e = P().filter((e) => e.focusable);
                      T(
                        [e.find((e) => e.active), e.find((e) => e.id === D), ...e]
                          .filter(Boolean)
                          .map((e) => e.ref.current),
                        m,
                      );
                    }
                  }
                  S.current = !1;
                }),
                onBlur: (0, o.mK)(e.onBlur, () => I(!1)),
              }),
            })
          );
        }),
        E = "RovingFocusGroupItem",
        P = t.forwardRef((e, r) => {
          let {
              __scopeRovingFocusGroup: n,
              focusable: a = !0,
              active: l = !1,
              tabStopId: u,
              children: d,
              ...s
            } = e,
            c = (0, v.B)(),
            p = u || c,
            f = A(E, n),
            h = f.currentTabStopId === p,
            m = j(n),
            { onFocusableItemAdd: g, onFocusableItemRemove: w, currentTabStopId: x } = f;
          return (
            t.useEffect(() => {
              if (a) return (g(), () => w());
            }, [a, g, w]),
            (0, y.jsx)(M.ItemSlot, {
              scope: n,
              id: p,
              focusable: a,
              active: l,
              children: (0, y.jsx)(i.sG.span, {
                tabIndex: h ? 0 : -1,
                "data-orientation": f.orientation,
                ...s,
                ref: r,
                onMouseDown: (0, o.mK)(e.onMouseDown, (e) => {
                  a ? f.onItemFocus(p) : e.preventDefault();
                }),
                onFocus: (0, o.mK)(e.onFocus, () => f.onItemFocus(p)),
                onKeyDown: (0, o.mK)(e.onKeyDown, (e) => {
                  if ("Tab" === e.key && e.shiftKey) return void f.onItemShiftTab();
                  if (e.target !== e.currentTarget) return;
                  let r = (function (e, r, n) {
                    var t;
                    let o =
                      ((t = e.key),
                      "rtl" !== n
                        ? t
                        : "ArrowLeft" === t
                          ? "ArrowRight"
                          : "ArrowRight" === t
                            ? "ArrowLeft"
                            : t);
                    if (
                      !("vertical" === r && ["ArrowLeft", "ArrowRight"].includes(o)) &&
                      !("horizontal" === r && ["ArrowUp", "ArrowDown"].includes(o))
                    )
                      return S[o];
                  })(e, f.orientation, f.dir);
                  if (void 0 !== r) {
                    if (e.metaKey || e.ctrlKey || e.altKey || e.shiftKey) return;
                    e.preventDefault();
                    let n = m()
                      .filter((e) => e.focusable)
                      .map((e) => e.ref.current);
                    if ("last" === r) n.reverse();
                    else if ("prev" === r || "next" === r) {
                      "prev" === r && n.reverse();
                      let t = n.indexOf(e.currentTarget);
                      n = f.loop
                        ? (function (e, r) {
                            return e.map((n, t) => e[(r + t) % e.length]);
                          })(n, t + 1)
                        : n.slice(t + 1);
                    }
                    setTimeout(() => T(n));
                  }
                }),
                children:
                  "function" == typeof d ? d({ isCurrentTabStop: h, hasTabStop: null != x }) : d,
              }),
            })
          );
        });
      P.displayName = E;
      var S = {
        ArrowLeft: "prev",
        ArrowUp: "prev",
        ArrowRight: "next",
        ArrowDown: "next",
        PageUp: "first",
        Home: "first",
        PageDown: "last",
        End: "last",
      };
      function T(e) {
        let r = arguments.length > 1 && void 0 !== arguments[1] && arguments[1],
          n = document.activeElement;
        for (let t of e)
          if (t === n || (t.focus({ preventScroll: r }), document.activeElement !== n)) return;
      }
      var N = Symbol("radix.slottable");
      function F(e) {
        return (
          t.isValidElement(e) &&
          "function" == typeof e.type &&
          "__radixId" in e.type &&
          e.type.__radixId === N
        );
      }
      var O = n(97745),
        L = n(40101),
        G = ["Enter", " "],
        B = ["ArrowUp", "PageDown", "End"],
        U = ["ArrowDown", "PageUp", "Home", ...B],
        V = { ltr: [...G, "ArrowRight"], rtl: [...G, "ArrowLeft"] },
        z = { ltr: ["ArrowLeft"], rtl: ["ArrowRight"] },
        H = "Menu",
        [W, X, Z] = (0, d.N)(H),
        [q, Y] = (0, l.A)(H, [Z, h.Bk, D]),
        J = (0, h.Bk)(),
        $ = D(),
        [Q, ee] = q(H),
        [er, en] = q(H),
        et = (e) => {
          let {
              __scopeMenu: r,
              open: n = !1,
              children: o,
              dir: a,
              onOpenChange: l,
              modal: u = !0,
            } = e,
            i = J(r),
            [d, c] = t.useState(null),
            p = t.useRef(!1),
            f = (0, w.c)(l),
            v = (0, s.jH)(a);
          return (
            t.useEffect(() => {
              let e = () => {
                  ((p.current = !0),
                    document.addEventListener("pointerdown", r, { capture: !0, once: !0 }),
                    document.addEventListener("pointermove", r, { capture: !0, once: !0 }));
                },
                r = () => (p.current = !1);
              return (
                document.addEventListener("keydown", e, { capture: !0 }),
                () => {
                  (document.removeEventListener("keydown", e, { capture: !0 }),
                    document.removeEventListener("pointerdown", r, { capture: !0 }),
                    document.removeEventListener("pointermove", r, { capture: !0 }));
                }
              );
            }, []),
            (0, y.jsx)(h.bL, {
              ...i,
              children: (0, y.jsx)(Q, {
                scope: r,
                open: n,
                onOpenChange: f,
                content: d,
                onContentChange: c,
                children: (0, y.jsx)(er, {
                  scope: r,
                  onClose: t.useCallback(() => f(!1), [f]),
                  isUsingKeyboardRef: p,
                  dir: v,
                  modal: u,
                  children: o,
                }),
              }),
            })
          );
        };
      et.displayName = H;
      var eo = t.forwardRef((e, r) => {
        let { __scopeMenu: n, ...t } = e,
          o = J(n);
        return (0, y.jsx)(h.Mz, { ...o, ...t, ref: r });
      });
      eo.displayName = "MenuAnchor";
      var ea = "MenuPortal",
        [el, eu] = q(ea, { forceMount: void 0 }),
        ei = (e) => {
          let { __scopeMenu: r, forceMount: n, children: t, container: o } = e,
            a = ee(ea, r);
          return (0, y.jsx)(el, {
            scope: r,
            forceMount: n,
            children: (0, y.jsx)(g.C, {
              present: n || a.open,
              children: (0, y.jsx)(m.Z, { asChild: !0, container: o, children: t }),
            }),
          });
        };
      ei.displayName = ea;
      var ed = "MenuContent",
        [es, ec] = q(ed),
        ep = t.forwardRef((e, r) => {
          let n = eu(ed, e.__scopeMenu),
            { forceMount: t = n.forceMount, ...o } = e,
            a = ee(ed, e.__scopeMenu),
            l = en(ed, e.__scopeMenu);
          return (0, y.jsx)(W.Provider, {
            scope: e.__scopeMenu,
            children: (0, y.jsx)(g.C, {
              present: t || a.open,
              children: (0, y.jsx)(W.Slot, {
                scope: e.__scopeMenu,
                children: l.modal
                  ? (0, y.jsx)(ef, { ...o, ref: r })
                  : (0, y.jsx)(ev, { ...o, ref: r }),
              }),
            }),
          });
        }),
        ef = t.forwardRef((e, r) => {
          let n = ee(ed, e.__scopeMenu),
            l = t.useRef(null),
            u = (0, a.s)(r, l);
          return (
            t.useEffect(() => {
              let e = l.current;
              if (e) return (0, O.Eq)(e);
            }, []),
            (0, y.jsx)(em, {
              ...e,
              ref: u,
              trapFocus: n.open,
              disableOutsidePointerEvents: n.open,
              disableOutsideScroll: !0,
              onFocusOutside: (0, o.mK)(e.onFocusOutside, (e) => e.preventDefault(), {
                checkForDefaultPrevented: !1,
              }),
              onDismiss: () => n.onOpenChange(!1),
            })
          );
        }),
        ev = t.forwardRef((e, r) => {
          let n = ee(ed, e.__scopeMenu);
          return (0, y.jsx)(em, {
            ...e,
            ref: r,
            trapFocus: !1,
            disableOutsidePointerEvents: !1,
            disableOutsideScroll: !1,
            onDismiss: () => n.onOpenChange(!1),
          });
        }),
        eh = (function (e) {
          let r = (function (e) {
              let r = t.forwardRef((e, r) => {
                let { children: n, ...o } = e;
                if (t.isValidElement(n)) {
                  var l;
                  let e,
                    u,
                    i =
                      ((l = n),
                      (u =
                        (e = Object.getOwnPropertyDescriptor(l.props, "ref")?.get) &&
                        "isReactWarning" in e &&
                        e.isReactWarning)
                        ? l.ref
                        : (u =
                              (e = Object.getOwnPropertyDescriptor(l, "ref")?.get) &&
                              "isReactWarning" in e &&
                              e.isReactWarning)
                          ? l.props.ref
                          : l.props.ref || l.ref),
                    d = (function (e, r) {
                      let n = { ...r };
                      for (let t in r) {
                        let o = e[t],
                          a = r[t];
                        /^on[A-Z]/.test(t)
                          ? o && a
                            ? (n[t] = (...e) => {
                                let r = a(...e);
                                return (o(...e), r);
                              })
                            : o && (n[t] = o)
                          : "style" === t
                            ? (n[t] = { ...o, ...a })
                            : "className" === t && (n[t] = [o, a].filter(Boolean).join(" "));
                      }
                      return { ...e, ...n };
                    })(o, n.props);
                  return (
                    n.type !== t.Fragment && (d.ref = r ? (0, a.t)(r, i) : i),
                    t.cloneElement(n, d)
                  );
                }
                return t.Children.count(n) > 1 ? t.Children.only(null) : null;
              });
              return ((r.displayName = `${e}.SlotClone`), r);
            })(e),
            n = t.forwardRef((e, n) => {
              let { children: o, ...a } = e,
                l = t.Children.toArray(o),
                u = l.find(F);
              if (u) {
                let e = u.props.children,
                  o = l.map((r) =>
                    r !== u
                      ? r
                      : t.Children.count(e) > 1
                        ? t.Children.only(null)
                        : t.isValidElement(e)
                          ? e.props.children
                          : null,
                  );
                return (0, y.jsx)(r, {
                  ...a,
                  ref: n,
                  children: t.isValidElement(e) ? t.cloneElement(e, void 0, o) : null,
                });
              }
              return (0, y.jsx)(r, { ...a, ref: n, children: o });
            });
          return ((n.displayName = `${e}.Slot`), n);
        })("MenuContent.ScrollLock"),
        em = t.forwardRef((e, r) => {
          let {
              __scopeMenu: n,
              loop: l = !1,
              trapFocus: u,
              onOpenAutoFocus: i,
              onCloseAutoFocus: d,
              disableOutsidePointerEvents: s,
              onEntryFocus: v,
              onEscapeKeyDown: m,
              onPointerDownOutside: g,
              onFocusOutside: w,
              onInteractOutside: x,
              onDismiss: b,
              disableOutsideScroll: C,
              ...M
            } = e,
            j = ee(ed, n),
            R = en(ed, n),
            k = J(n),
            D = $(n),
            _ = X(n),
            [A, I] = t.useState(null),
            E = t.useRef(null),
            P = (0, a.s)(r, E, j.onContentChange),
            S = t.useRef(0),
            T = t.useRef(""),
            N = t.useRef(0),
            F = t.useRef(null),
            O = t.useRef("right"),
            G = t.useRef(0),
            V = C ? L.A : t.Fragment;
          (t.useEffect(() => () => window.clearTimeout(S.current), []), (0, p.Oh)());
          let z = t.useCallback((e) => {
            var r, n;
            return (
              O.current === (null == (r = F.current) ? void 0 : r.side) &&
              (function (e, r) {
                return (
                  !!r &&
                  (function (e, r) {
                    let { x: n, y: t } = e,
                      o = !1;
                    for (let e = 0, a = r.length - 1; e < r.length; a = e++) {
                      let l = r[e],
                        u = r[a],
                        i = l.x,
                        d = l.y,
                        s = u.x,
                        c = u.y;
                      d > t != c > t && n < ((s - i) * (t - d)) / (c - d) + i && (o = !o);
                    }
                    return o;
                  })({ x: e.clientX, y: e.clientY }, r)
                );
              })(e, null == (n = F.current) ? void 0 : n.area)
            );
          }, []);
          return (0, y.jsx)(es, {
            scope: n,
            searchRef: T,
            onItemEnter: t.useCallback(
              (e) => {
                z(e) && e.preventDefault();
              },
              [z],
            ),
            onItemLeave: t.useCallback(
              (e) => {
                var r;
                z(e) || (null == (r = E.current) || r.focus(), I(null));
              },
              [z],
            ),
            onTriggerLeave: t.useCallback(
              (e) => {
                z(e) && e.preventDefault();
              },
              [z],
            ),
            pointerGraceTimerRef: N,
            onPointerGraceIntentChange: t.useCallback((e) => {
              F.current = e;
            }, []),
            children: (0, y.jsx)(V, {
              ...(C ? { as: eh, allowPinchZoom: !0 } : void 0),
              children: (0, y.jsx)(f.n, {
                asChild: !0,
                trapped: u,
                onMountAutoFocus: (0, o.mK)(i, (e) => {
                  var r;
                  (e.preventDefault(), null == (r = E.current) || r.focus({ preventScroll: !0 }));
                }),
                onUnmountAutoFocus: d,
                children: (0, y.jsx)(c.qW, {
                  asChild: !0,
                  disableOutsidePointerEvents: s,
                  onEscapeKeyDown: m,
                  onPointerDownOutside: g,
                  onFocusOutside: w,
                  onInteractOutside: x,
                  onDismiss: b,
                  children: (0, y.jsx)(K, {
                    asChild: !0,
                    ...D,
                    dir: R.dir,
                    orientation: "vertical",
                    loop: l,
                    currentTabStopId: A,
                    onCurrentTabStopIdChange: I,
                    onEntryFocus: (0, o.mK)(v, (e) => {
                      R.isUsingKeyboardRef.current || e.preventDefault();
                    }),
                    preventScrollOnEntryFocus: !0,
                    children: (0, y.jsx)(h.UC, {
                      role: "menu",
                      "aria-orientation": "vertical",
                      "data-state": ez(j.open),
                      "data-radix-menu-content": "",
                      dir: R.dir,
                      ...k,
                      ...M,
                      ref: P,
                      style: { outline: "none", ...M.style },
                      onKeyDown: (0, o.mK)(M.onKeyDown, (e) => {
                        let r = e.target.closest("[data-radix-menu-content]") === e.currentTarget,
                          n = e.ctrlKey || e.altKey || e.metaKey,
                          t = 1 === e.key.length;
                        r &&
                          ("Tab" === e.key && e.preventDefault(),
                          !n &&
                            t &&
                            ((e) => {
                              var r, n;
                              let t = T.current + e,
                                o = _().filter((e) => !e.disabled),
                                a = document.activeElement,
                                l =
                                  null == (r = o.find((e) => e.ref.current === a))
                                    ? void 0
                                    : r.textValue,
                                u = (function (e, r, n) {
                                  var t;
                                  let o =
                                      r.length > 1 && Array.from(r).every((e) => e === r[0])
                                        ? r[0]
                                        : r,
                                    a = n ? e.indexOf(n) : -1,
                                    l =
                                      ((t = Math.max(a, 0)),
                                      e.map((r, n) => e[(t + n) % e.length]));
                                  1 === o.length && (l = l.filter((e) => e !== n));
                                  let u = l.find((e) =>
                                    e.toLowerCase().startsWith(o.toLowerCase()),
                                  );
                                  return u !== n ? u : void 0;
                                })(
                                  o.map((e) => e.textValue),
                                  t,
                                  l,
                                ),
                                i =
                                  null == (n = o.find((e) => e.textValue === u))
                                    ? void 0
                                    : n.ref.current;
                              (!(function e(r) {
                                ((T.current = r),
                                  window.clearTimeout(S.current),
                                  "" !== r && (S.current = window.setTimeout(() => e(""), 1e3)));
                              })(t),
                                i && setTimeout(() => i.focus()));
                            })(e.key));
                        let o = E.current;
                        if (e.target !== o || !U.includes(e.key)) return;
                        e.preventDefault();
                        let a = _()
                          .filter((e) => !e.disabled)
                          .map((e) => e.ref.current);
                        (B.includes(e.key) && a.reverse(),
                          (function (e) {
                            let r = document.activeElement;
                            for (let n of e)
                              if (n === r || (n.focus(), document.activeElement !== r)) return;
                          })(a));
                      }),
                      onBlur: (0, o.mK)(e.onBlur, (e) => {
                        e.currentTarget.contains(e.target) ||
                          (window.clearTimeout(S.current), (T.current = ""));
                      }),
                      onPointerMove: (0, o.mK)(
                        e.onPointerMove,
                        eX((e) => {
                          let r = e.target,
                            n = G.current !== e.clientX;
                          e.currentTarget.contains(r) &&
                            n &&
                            ((O.current = e.clientX > G.current ? "right" : "left"),
                            (G.current = e.clientX));
                        }),
                      ),
                    }),
                  }),
                }),
              }),
            }),
          });
        });
      ep.displayName = ed;
      var eg = t.forwardRef((e, r) => {
        let { __scopeMenu: n, ...t } = e;
        return (0, y.jsx)(i.sG.div, { role: "group", ...t, ref: r });
      });
      eg.displayName = "MenuGroup";
      var ew = t.forwardRef((e, r) => {
        let { __scopeMenu: n, ...t } = e;
        return (0, y.jsx)(i.sG.div, { ...t, ref: r });
      });
      ew.displayName = "MenuLabel";
      var ey = "MenuItem",
        ex = "menu.itemSelect",
        eb = t.forwardRef((e, r) => {
          let { disabled: n = !1, onSelect: l, ...u } = e,
            d = t.useRef(null),
            s = en(ey, e.__scopeMenu),
            c = ec(ey, e.__scopeMenu),
            p = (0, a.s)(r, d),
            f = t.useRef(!1);
          return (0, y.jsx)(eC, {
            ...u,
            ref: p,
            disabled: n,
            onClick: (0, o.mK)(e.onClick, () => {
              let e = d.current;
              if (!n && e) {
                let r = new CustomEvent(ex, { bubbles: !0, cancelable: !0 });
                (e.addEventListener(ex, (e) => (null == l ? void 0 : l(e)), { once: !0 }),
                  (0, i.hO)(e, r),
                  r.defaultPrevented ? (f.current = !1) : s.onClose());
              }
            }),
            onPointerDown: (r) => {
              var n;
              (null == (n = e.onPointerDown) || n.call(e, r), (f.current = !0));
            },
            onPointerUp: (0, o.mK)(e.onPointerUp, (e) => {
              var r;
              f.current || null == (r = e.currentTarget) || r.click();
            }),
            onKeyDown: (0, o.mK)(e.onKeyDown, (e) => {
              let r = "" !== c.searchRef.current;
              n ||
                (r && " " === e.key) ||
                (G.includes(e.key) && (e.currentTarget.click(), e.preventDefault()));
            }),
          });
        });
      eb.displayName = ey;
      var eC = t.forwardRef((e, r) => {
          let { __scopeMenu: n, disabled: l = !1, textValue: u, ...d } = e,
            s = ec(ey, n),
            c = $(n),
            p = t.useRef(null),
            f = (0, a.s)(r, p),
            [v, h] = t.useState(!1),
            [m, g] = t.useState("");
          return (
            t.useEffect(() => {
              let e = p.current;
              if (e) {
                var r;
                g((null != (r = e.textContent) ? r : "").trim());
              }
            }, [d.children]),
            (0, y.jsx)(W.ItemSlot, {
              scope: n,
              disabled: l,
              textValue: null != u ? u : m,
              children: (0, y.jsx)(P, {
                asChild: !0,
                ...c,
                focusable: !l,
                children: (0, y.jsx)(i.sG.div, {
                  role: "menuitem",
                  "data-highlighted": v ? "" : void 0,
                  "aria-disabled": l || void 0,
                  "data-disabled": l ? "" : void 0,
                  ...d,
                  ref: f,
                  onPointerMove: (0, o.mK)(
                    e.onPointerMove,
                    eX((e) => {
                      l
                        ? s.onItemLeave(e)
                        : (s.onItemEnter(e),
                          e.defaultPrevented || e.currentTarget.focus({ preventScroll: !0 }));
                    }),
                  ),
                  onPointerLeave: (0, o.mK)(
                    e.onPointerLeave,
                    eX((e) => s.onItemLeave(e)),
                  ),
                  onFocus: (0, o.mK)(e.onFocus, () => h(!0)),
                  onBlur: (0, o.mK)(e.onBlur, () => h(!1)),
                }),
              }),
            })
          );
        }),
        eM = t.forwardRef((e, r) => {
          let { checked: n = !1, onCheckedChange: t, ...a } = e;
          return (0, y.jsx)(eI, {
            scope: e.__scopeMenu,
            checked: n,
            children: (0, y.jsx)(eb, {
              role: "menuitemcheckbox",
              "aria-checked": eH(n) ? "mixed" : n,
              ...a,
              ref: r,
              "data-state": eW(n),
              onSelect: (0, o.mK)(a.onSelect, () => (null == t ? void 0 : t(!!eH(n) || !n)), {
                checkForDefaultPrevented: !1,
              }),
            }),
          });
        });
      eM.displayName = "MenuCheckboxItem";
      var ej = "MenuRadioGroup",
        [eR, ek] = q(ej, { value: void 0, onValueChange: () => {} }),
        eD = t.forwardRef((e, r) => {
          let { value: n, onValueChange: t, ...o } = e,
            a = (0, w.c)(t);
          return (0, y.jsx)(eR, {
            scope: e.__scopeMenu,
            value: n,
            onValueChange: a,
            children: (0, y.jsx)(eg, { ...o, ref: r }),
          });
        });
      eD.displayName = ej;
      var e_ = "MenuRadioItem",
        eA = t.forwardRef((e, r) => {
          let { value: n, ...t } = e,
            a = ek(e_, e.__scopeMenu),
            l = n === a.value;
          return (0, y.jsx)(eI, {
            scope: e.__scopeMenu,
            checked: l,
            children: (0, y.jsx)(eb, {
              role: "menuitemradio",
              "aria-checked": l,
              ...t,
              ref: r,
              "data-state": eW(l),
              onSelect: (0, o.mK)(
                t.onSelect,
                () => {
                  var e;
                  return null == (e = a.onValueChange) ? void 0 : e.call(a, n);
                },
                { checkForDefaultPrevented: !1 },
              ),
            }),
          });
        });
      eA.displayName = e_;
      var eK = "MenuItemIndicator",
        [eI, eE] = q(eK, { checked: !1 }),
        eP = t.forwardRef((e, r) => {
          let { __scopeMenu: n, forceMount: t, ...o } = e,
            a = eE(eK, n);
          return (0, y.jsx)(g.C, {
            present: t || eH(a.checked) || !0 === a.checked,
            children: (0, y.jsx)(i.sG.span, { ...o, ref: r, "data-state": eW(a.checked) }),
          });
        });
      eP.displayName = eK;
      var eS = t.forwardRef((e, r) => {
        let { __scopeMenu: n, ...t } = e;
        return (0, y.jsx)(i.sG.div, {
          role: "separator",
          "aria-orientation": "horizontal",
          ...t,
          ref: r,
        });
      });
      eS.displayName = "MenuSeparator";
      var eT = t.forwardRef((e, r) => {
        let { __scopeMenu: n, ...t } = e,
          o = J(n);
        return (0, y.jsx)(h.i3, { ...o, ...t, ref: r });
      });
      eT.displayName = "MenuArrow";
      var eN = "MenuSub",
        [eF, eO] = q(eN),
        eL = (e) => {
          let { __scopeMenu: r, children: n, open: o = !1, onOpenChange: a } = e,
            l = ee(eN, r),
            u = J(r),
            [i, d] = t.useState(null),
            [s, c] = t.useState(null),
            p = (0, w.c)(a);
          return (
            t.useEffect(() => (!1 === l.open && p(!1), () => p(!1)), [l.open, p]),
            (0, y.jsx)(h.bL, {
              ...u,
              children: (0, y.jsx)(Q, {
                scope: r,
                open: o,
                onOpenChange: p,
                content: s,
                onContentChange: c,
                children: (0, y.jsx)(eF, {
                  scope: r,
                  contentId: (0, v.B)(),
                  triggerId: (0, v.B)(),
                  trigger: i,
                  onTriggerChange: d,
                  children: n,
                }),
              }),
            })
          );
        };
      eL.displayName = eN;
      var eG = "MenuSubTrigger",
        eB = t.forwardRef((e, r) => {
          let n = ee(eG, e.__scopeMenu),
            l = en(eG, e.__scopeMenu),
            u = eO(eG, e.__scopeMenu),
            i = ec(eG, e.__scopeMenu),
            d = t.useRef(null),
            { pointerGraceTimerRef: s, onPointerGraceIntentChange: c } = i,
            p = { __scopeMenu: e.__scopeMenu },
            f = t.useCallback(() => {
              (d.current && window.clearTimeout(d.current), (d.current = null));
            }, []);
          return (
            t.useEffect(() => f, [f]),
            t.useEffect(() => {
              let e = s.current;
              return () => {
                (window.clearTimeout(e), c(null));
              };
            }, [s, c]),
            (0, y.jsx)(eo, {
              asChild: !0,
              ...p,
              children: (0, y.jsx)(eC, {
                id: u.triggerId,
                "aria-haspopup": "menu",
                "aria-expanded": n.open,
                "aria-controls": u.contentId,
                "data-state": ez(n.open),
                ...e,
                ref: (0, a.t)(r, u.onTriggerChange),
                onClick: (r) => {
                  var t;
                  (null == (t = e.onClick) || t.call(e, r),
                    e.disabled ||
                      r.defaultPrevented ||
                      (r.currentTarget.focus(), n.open || n.onOpenChange(!0)));
                },
                onPointerMove: (0, o.mK)(
                  e.onPointerMove,
                  eX((r) => {
                    (i.onItemEnter(r),
                      !r.defaultPrevented &&
                        (e.disabled ||
                          n.open ||
                          d.current ||
                          (i.onPointerGraceIntentChange(null),
                          (d.current = window.setTimeout(() => {
                            (n.onOpenChange(!0), f());
                          }, 100)))));
                  }),
                ),
                onPointerLeave: (0, o.mK)(
                  e.onPointerLeave,
                  eX((e) => {
                    var r, t;
                    f();
                    let o = null == (r = n.content) ? void 0 : r.getBoundingClientRect();
                    if (o) {
                      let r = null == (t = n.content) ? void 0 : t.dataset.side,
                        a = "right" === r,
                        l = o[a ? "left" : "right"],
                        u = o[a ? "right" : "left"];
                      (i.onPointerGraceIntentChange({
                        area: [
                          { x: e.clientX + (a ? -5 : 5), y: e.clientY },
                          { x: l, y: o.top },
                          { x: u, y: o.top },
                          { x: u, y: o.bottom },
                          { x: l, y: o.bottom },
                        ],
                        side: r,
                      }),
                        window.clearTimeout(s.current),
                        (s.current = window.setTimeout(
                          () => i.onPointerGraceIntentChange(null),
                          300,
                        )));
                    } else {
                      if ((i.onTriggerLeave(e), e.defaultPrevented)) return;
                      i.onPointerGraceIntentChange(null);
                    }
                  }),
                ),
                onKeyDown: (0, o.mK)(e.onKeyDown, (r) => {
                  let t = "" !== i.searchRef.current;
                  if (!e.disabled && (!t || " " !== r.key) && V[l.dir].includes(r.key)) {
                    var o;
                    (n.onOpenChange(!0), null == (o = n.content) || o.focus(), r.preventDefault());
                  }
                }),
              }),
            })
          );
        });
      eB.displayName = eG;
      var eU = "MenuSubContent",
        eV = t.forwardRef((e, r) => {
          let n = eu(ed, e.__scopeMenu),
            { forceMount: l = n.forceMount, ...u } = e,
            i = ee(ed, e.__scopeMenu),
            d = en(ed, e.__scopeMenu),
            s = eO(eU, e.__scopeMenu),
            c = t.useRef(null),
            p = (0, a.s)(r, c);
          return (0, y.jsx)(W.Provider, {
            scope: e.__scopeMenu,
            children: (0, y.jsx)(g.C, {
              present: l || i.open,
              children: (0, y.jsx)(W.Slot, {
                scope: e.__scopeMenu,
                children: (0, y.jsx)(em, {
                  id: s.contentId,
                  "aria-labelledby": s.triggerId,
                  ...u,
                  ref: p,
                  align: "start",
                  side: "rtl" === d.dir ? "left" : "right",
                  disableOutsidePointerEvents: !1,
                  disableOutsideScroll: !1,
                  trapFocus: !1,
                  onOpenAutoFocus: (e) => {
                    var r;
                    (d.isUsingKeyboardRef.current && (null == (r = c.current) || r.focus()),
                      e.preventDefault());
                  },
                  onCloseAutoFocus: (e) => e.preventDefault(),
                  onFocusOutside: (0, o.mK)(e.onFocusOutside, (e) => {
                    e.target !== s.trigger && i.onOpenChange(!1);
                  }),
                  onEscapeKeyDown: (0, o.mK)(e.onEscapeKeyDown, (e) => {
                    (d.onClose(), e.preventDefault());
                  }),
                  onKeyDown: (0, o.mK)(e.onKeyDown, (e) => {
                    let r = e.currentTarget.contains(e.target),
                      n = z[d.dir].includes(e.key);
                    if (r && n) {
                      var t;
                      (i.onOpenChange(!1),
                        null == (t = s.trigger) || t.focus(),
                        e.preventDefault());
                    }
                  }),
                }),
              }),
            }),
          });
        });
      function ez(e) {
        return e ? "open" : "closed";
      }
      function eH(e) {
        return "indeterminate" === e;
      }
      function eW(e) {
        return eH(e) ? "indeterminate" : e ? "checked" : "unchecked";
      }
      function eX(e) {
        return (r) => ("mouse" === r.pointerType ? e(r) : void 0);
      }
      eV.displayName = eU;
      var eZ = "DropdownMenu",
        [eq, eY] = (0, l.A)(eZ, [Y]),
        eJ = Y(),
        [e$, eQ] = eq(eZ),
        e0 = (e) => {
          let {
              __scopeDropdownMenu: r,
              children: n,
              dir: o,
              open: a,
              defaultOpen: l,
              onOpenChange: i,
              modal: d = !0,
            } = e,
            s = eJ(r),
            c = t.useRef(null),
            [p, f] = (0, u.i)({ prop: a, defaultProp: null != l && l, onChange: i, caller: eZ });
          return (0, y.jsx)(e$, {
            scope: r,
            triggerId: (0, v.B)(),
            triggerRef: c,
            contentId: (0, v.B)(),
            open: p,
            onOpenChange: f,
            onOpenToggle: t.useCallback(() => f((e) => !e), [f]),
            modal: d,
            children: (0, y.jsx)(et, {
              ...s,
              open: p,
              onOpenChange: f,
              dir: o,
              modal: d,
              children: n,
            }),
          });
        };
      e0.displayName = eZ;
      var e1 = "DropdownMenuTrigger",
        e2 = t.forwardRef((e, r) => {
          let { __scopeDropdownMenu: n, disabled: t = !1, ...l } = e,
            u = eQ(e1, n),
            d = eJ(n);
          return (0, y.jsx)(eo, {
            asChild: !0,
            ...d,
            children: (0, y.jsx)(i.sG.button, {
              type: "button",
              id: u.triggerId,
              "aria-haspopup": "menu",
              "aria-expanded": u.open,
              "aria-controls": u.open ? u.contentId : void 0,
              "data-state": u.open ? "open" : "closed",
              "data-disabled": t ? "" : void 0,
              disabled: t,
              ...l,
              ref: (0, a.t)(r, u.triggerRef),
              onPointerDown: (0, o.mK)(e.onPointerDown, (e) => {
                !t &&
                  0 === e.button &&
                  !1 === e.ctrlKey &&
                  (u.onOpenToggle(), u.open || e.preventDefault());
              }),
              onKeyDown: (0, o.mK)(e.onKeyDown, (e) => {
                !t &&
                  (["Enter", " "].includes(e.key) && u.onOpenToggle(),
                  "ArrowDown" === e.key && u.onOpenChange(!0),
                  ["Enter", " ", "ArrowDown"].includes(e.key) && e.preventDefault());
              }),
            }),
          });
        });
      e2.displayName = e1;
      var e3 = (e) => {
        let { __scopeDropdownMenu: r, ...n } = e,
          t = eJ(r);
        return (0, y.jsx)(ei, { ...t, ...n });
      };
      e3.displayName = "DropdownMenuPortal";
      var e4 = "DropdownMenuContent",
        e5 = t.forwardRef((e, r) => {
          let { __scopeDropdownMenu: n, ...a } = e,
            l = eQ(e4, n),
            u = eJ(n),
            i = t.useRef(!1);
          return (0, y.jsx)(ep, {
            id: l.contentId,
            "aria-labelledby": l.triggerId,
            ...u,
            ...a,
            ref: r,
            onCloseAutoFocus: (0, o.mK)(e.onCloseAutoFocus, (e) => {
              var r;
              (i.current || null == (r = l.triggerRef.current) || r.focus(),
                (i.current = !1),
                e.preventDefault());
            }),
            onInteractOutside: (0, o.mK)(e.onInteractOutside, (e) => {
              let r = e.detail.originalEvent,
                n = 0 === r.button && !0 === r.ctrlKey,
                t = 2 === r.button || n;
              (!l.modal || t) && (i.current = !0);
            }),
            style: {
              ...e.style,
              "--radix-dropdown-menu-content-transform-origin":
                "var(--radix-popper-transform-origin)",
              "--radix-dropdown-menu-content-available-width":
                "var(--radix-popper-available-width)",
              "--radix-dropdown-menu-content-available-height":
                "var(--radix-popper-available-height)",
              "--radix-dropdown-menu-trigger-width": "var(--radix-popper-anchor-width)",
              "--radix-dropdown-menu-trigger-height": "var(--radix-popper-anchor-height)",
            },
          });
        });
      e5.displayName = e4;
      var e9 = t.forwardRef((e, r) => {
        let { __scopeDropdownMenu: n, ...t } = e,
          o = eJ(n);
        return (0, y.jsx)(eg, { ...o, ...t, ref: r });
      });
      e9.displayName = "DropdownMenuGroup";
      var e6 = t.forwardRef((e, r) => {
        let { __scopeDropdownMenu: n, ...t } = e,
          o = eJ(n);
        return (0, y.jsx)(ew, { ...o, ...t, ref: r });
      });
      e6.displayName = "DropdownMenuLabel";
      var e8 = t.forwardRef((e, r) => {
        let { __scopeDropdownMenu: n, ...t } = e,
          o = eJ(n);
        return (0, y.jsx)(eb, { ...o, ...t, ref: r });
      });
      e8.displayName = "DropdownMenuItem";
      var e7 = t.forwardRef((e, r) => {
        let { __scopeDropdownMenu: n, ...t } = e,
          o = eJ(n);
        return (0, y.jsx)(eM, { ...o, ...t, ref: r });
      });
      e7.displayName = "DropdownMenuCheckboxItem";
      var re = t.forwardRef((e, r) => {
        let { __scopeDropdownMenu: n, ...t } = e,
          o = eJ(n);
        return (0, y.jsx)(eD, { ...o, ...t, ref: r });
      });
      re.displayName = "DropdownMenuRadioGroup";
      var rr = t.forwardRef((e, r) => {
        let { __scopeDropdownMenu: n, ...t } = e,
          o = eJ(n);
        return (0, y.jsx)(eA, { ...o, ...t, ref: r });
      });
      rr.displayName = "DropdownMenuRadioItem";
      var rn = t.forwardRef((e, r) => {
        let { __scopeDropdownMenu: n, ...t } = e,
          o = eJ(n);
        return (0, y.jsx)(eP, { ...o, ...t, ref: r });
      });
      rn.displayName = "DropdownMenuItemIndicator";
      var rt = t.forwardRef((e, r) => {
        let { __scopeDropdownMenu: n, ...t } = e,
          o = eJ(n);
        return (0, y.jsx)(eS, { ...o, ...t, ref: r });
      });
      ((rt.displayName = "DropdownMenuSeparator"),
        (t.forwardRef((e, r) => {
          let { __scopeDropdownMenu: n, ...t } = e,
            o = eJ(n);
          return (0, y.jsx)(eT, { ...o, ...t, ref: r });
        }).displayName = "DropdownMenuArrow"));
      var ro = t.forwardRef((e, r) => {
        let { __scopeDropdownMenu: n, ...t } = e,
          o = eJ(n);
        return (0, y.jsx)(eB, { ...o, ...t, ref: r });
      });
      ro.displayName = "DropdownMenuSubTrigger";
      var ra = t.forwardRef((e, r) => {
        let { __scopeDropdownMenu: n, ...t } = e,
          o = eJ(n);
        return (0, y.jsx)(eV, {
          ...o,
          ...t,
          ref: r,
          style: {
            ...e.style,
            "--radix-dropdown-menu-content-transform-origin":
              "var(--radix-popper-transform-origin)",
            "--radix-dropdown-menu-content-available-width": "var(--radix-popper-available-width)",
            "--radix-dropdown-menu-content-available-height":
              "var(--radix-popper-available-height)",
            "--radix-dropdown-menu-trigger-width": "var(--radix-popper-anchor-width)",
            "--radix-dropdown-menu-trigger-height": "var(--radix-popper-anchor-height)",
          },
        });
      });
      ra.displayName = "DropdownMenuSubContent";
      var rl = e0,
        ru = e2,
        ri = e3,
        rd = e5,
        rs = e9,
        rc = e6,
        rp = e8,
        rf = e7,
        rv = re,
        rh = rr,
        rm = rn,
        rg = rt,
        rw = (e) => {
          let { __scopeDropdownMenu: r, children: n, open: t, onOpenChange: o, defaultOpen: a } = e,
            l = eJ(r),
            [i, d] = (0, u.i)({
              prop: t,
              defaultProp: null != a && a,
              onChange: o,
              caller: "DropdownMenuSub",
            });
          return (0, y.jsx)(eL, { ...l, open: i, onOpenChange: d, children: n });
        },
        ry = ro,
        rx = ra;
    },
    84926: (e, r, n) => {
      n.d(r, { A: () => t });
      let t = (0, n(5121).A)("log-out", [
        ["path", { d: "m16 17 5-5-5-5", key: "1bji2h" }],
        ["path", { d: "M21 12H9", key: "dn1m92" }],
        ["path", { d: "M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4", key: "1uf3rs" }],
      ]);
    },
    85998: (e, r, n) => {
      n.d(r, { A: () => t });
      let t = (0, n(5121).A)("bell", [
        ["path", { d: "M10.268 21a2 2 0 0 0 3.464 0", key: "vwvbt9" }],
        [
          "path",
          {
            d: "M3.262 15.326A1 1 0 0 0 4 17h16a1 1 0 0 0 .74-1.673C19.41 13.956 18 12.499 18 8A6 6 0 0 0 6 8c0 4.499-1.411 5.956-2.738 7.326",
            key: "11g9vi",
          },
        ],
      ]);
    },
    86651: (e, r, n) => {
      n.d(r, { A: () => t });
      let t = (0, n(5121).A)("search", [
        ["path", { d: "m21 21-4.34-4.34", key: "14j7rj" }],
        ["circle", { cx: "11", cy: "11", r: "8", key: "4ej97u" }],
      ]);
    },
    87454: (e, r, n) => {
      n.d(r, { A: () => t });
      let t = (0, n(5121).A)("chart-pie", [
        [
          "path",
          {
            d: "M21 12c.552 0 1.005-.449.95-.998a10 10 0 0 0-8.953-8.951c-.55-.055-.998.398-.998.95v8a1 1 0 0 0 1 1z",
            key: "pzmjnu",
          },
        ],
        ["path", { d: "M21.21 15.89A10 10 0 1 1 8 2.83", key: "k2fpak" }],
      ]);
    },
    99051: (e, r, n) => {
      n.d(r, { A: () => t });
      let t = (0, n(5121).A)("circle", [
        ["circle", { cx: "12", cy: "12", r: "10", key: "1mglay" }],
      ]);
    },
  },
]);
