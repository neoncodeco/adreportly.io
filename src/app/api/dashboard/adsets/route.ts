...
import {
  buildDashboardAdSetRowsFromFacebook,
  type DashboardAdSetBreakdownRow,
...
function matchesSearch(row: DashboardAdSetBreakdownRow, q: string) {
  if (!q) return true;
  return [row.name, row.id, row.code, row.campaignName, row.campaignId ?? "", row.accountName].some(
    (value) => value.toLowerCase().includes(q),
  );
}

function sortRows(rows: DashboardAdSetBreakdownRow[], sortKey: string) {
...
export async function GET(req: NextRequest) {
  ...
  try {
    const payload = await getOrSetCache(
      `user:dashboard-adsets:v2:${agencyId}:${datePreset}`,
      20_000,
      async () => {
        ...
        if (!token) {
          return {
            ...
            connected: false,
            currencySymbol: "৳",
            adsets: [] as DashboardAdSetBreakdownRow[],
          };
        }
        ...
        return buildDashboardAdSetRowsFromFacebook(
          token,
    ...
    const filtered = payload.adsets.filter((row) => {
    ...
    return NextResponse.json(
      {
        ...payload,
        adsets: sorted.slice(start, start + limit),
